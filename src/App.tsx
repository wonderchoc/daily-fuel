...
interface ProteinIntake { wheyProtein: number | ""; eggs: number | ""; chicken: number | ""; pork: number | ""; beef: number | ""; fish: number | ""; tofu: number | ""; otherProtein: number | ""; }
...
const defaultProteinIntake: ProteinIntake = { wheyProtein: "", eggs: "", chicken: "", pork: "", beef: "", fish: "", tofu: "", otherProtein: "" };
...
function App() {
  ...
  const [proteinIntake, setProteinIntake] = useState<ProteinIntake>(() => ({ ...defaultProteinIntake, ...safelyLoad(PROTEIN_INTAKE_KEY, defaultProteinIntake, isProteinIntake) }));
  ...
  function updateCaloriesBurned(value: string) { setTraining((current) => ({ ...current, caloriesBurned: value === "" ? "" : Number(value) })); setResults(null); }
  ...
  return <main className="page"><section className="calculator">
    ...
    {tab === "today" && <>
      ...
      <section className="panel workout-panel"><div className="section-heading"><span>🐻</span><div><h2>Today&apos;s workout</h2><p>Tell Fuel Bear what you&apos;re doing.</p></div></div><div className="form-grid workout-form-grid">
        ...
        <label className="full-width compact-calories">Calories burned<input type="number" min="0" max="3000" inputMode="numeric" value={training.caloriesBurned} placeholder={`${estimatedBurn} kcal estimated`} onChange={(event) => updateCaloriesBurned(event.target.value)} /><small className="field-note">Suggested: {estimatedBurn} kcal. Leave blank to calculate with this estimate, or <button type="button" onClick={() => updateCaloriesBurned(String(estimatedBurn))}>use estimate</button>.</small></label>
    ...
    {tab === "plan" && (results ? <section className="results" aria-live="polite"><div className="results-heading"><div><p className="eyebrow">TODAY&apos;S PLAN</p><h2>{profile.name ? `${profile.name}'s fuel` : "Your daily fuel"}</h2></div><strong>{results.calories} kcal</strong></div><div className="macro-grid"><article><span>Protein</span><strong>{results.protein} g</strong></article><article><span>Carbs</span><strong>{results.carbs} g</strong></article><article><span>Fat</span><strong>{results.fat} g</strong></article></div>
      ...
      <div className="estimate-carbs"><div className="estimate-protein-heading"><div><p className="eyebrow">CARB PORTIONS</p><h3>~{Math.round(results.carbs / 3)} g per main meal</h3></div><span>{results.carbs} g/day</span></div><p className="estimate-help">Swipe for meal guides. Beer is shown only for comparison—not as workout fuel.</p><div className="visual-portions carb-portions"><article><span>🍚</span><small>Cooked rice</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.rice * 10) * 10} g</strong></article><article><span>🍜</span><small>Noodles</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.noodles * 10) * 10} g</strong></article><article><span>🥔</span><small>Potato</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.potato * 10) * 10} g</strong></article><article><span>🥣</span><small>Dry oats</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.oats * 5) * 5} g</strong></article><article><span>🍌</span><small>Banana</small><strong>~{Math.max(1, Math.round((results.carbs / 3) / 23 * 2) / 2)}</strong><em>medium</em></article><article className="beer-card"><span>🍺</span><small>Regular beer</small><strong>330 ml</strong><em>~12 g carbs + alcohol</em></article></div></div>
