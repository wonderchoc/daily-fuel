  ...
  return <main className="page"><section className="calculator">
    ...
    {tab === "workout" ? <>
      ...
      <section className="panel"><div className="section-heading"><span>01</span><div><h2>Today&apos;s training</h2><p>Adjust what is different today.</p></div></div><div className="form-grid">
        ...
        <fieldset className="full-width segmented-field"><legend>Sweat level</legend><div className="segmented-buttons">{(["low", "medium", "high"] as Sweat[]).map((level) => <button type="button" className={training.sweat === level ? "active" : ""} aria-pressed={training.sweat === level} key={level} onClick={() => updateTraining("sweat", level)}>{level[0].toUpperCase() + level.slice(1)}</button>)}</div></fieldset>
