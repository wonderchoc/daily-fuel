...
function App() {
  ...
  return <main className="page"><section className="calculator">
    ...
    {tab === "workout" ? <>
      ...
      <section className="panel workout-panel"><div className="section-heading"><span>01</span><div><h2>Today&apos;s training</h2><p>Adjust what is different today.</p></div></div><div className="form-grid workout-form-grid">
        ...
        <label className="full-width duration-control"><span className="control-heading"><span>Duration</span><strong>{training.duration} min</strong></span><input type="range" min="10" max="180" step="5" value={training.duration} onChange={(event) => updateTraining("duration", Number(event.target.value))} aria-label="Workout duration in minutes" /><span className="range-labels"><small>10 min</small><small>180 min</small></span></label>
        <fieldset className="segmented-field"><legend>Intensity</legend><div className="segmented-buttons">{(["easy", "moderate", "hard"] as Intensity[]).map((level) => <button type="button" className={training.intensity === level ? "active" : ""} aria-pressed={training.intensity === level} key={level} onClick={() => updateTraining("intensity", level)}>{level === "moderate" ? "Mod" : level[0].toUpperCase() + level.slice(1)}</button>)}</div></fieldset>
        <fieldset className="segmented-field"><legend>Sweat</legend><div className="segmented-buttons">{(["low", "medium", "high"] as Sweat[]).map((level) => <button type="button" className={training.sweat === level ? "active" : ""} aria-pressed={training.sweat === level} key={level} onClick={() => updateTraining("sweat", level)}>{level[0].toUpperCase() + level.slice(1)}</button>)}</div></fieldset>
      ...
      </div><button className="reset-intake" type="button" onClick={() => setProteinIntake(defaultProteinIntake)}>Reset today&apos;s intake</button>
      {proteinTarget === null ? <button className="protein-setup" type="button" onClick={() => setTab("profile")}>Add your weight and goal to see a recommended protein target →</button> : <div className="live-protein-plan"><div className="protein-summary"><article><span>Recommended</span><strong>{proteinTarget} g</strong></article><article><span>Logged</span><strong>{totalProteinEaten} g</strong></article><article><span>Remaining</span><strong>{remainingProtein} g</strong></article></div><div className="progress-meta"><span>Daily protein progress</span><strong>{proteinProgressPercent}%</strong></div><progress className="protein-progress" max={proteinTarget} value={Math.min(proteinTarget, totalProteinEaten)} aria-label={`${totalProteinEaten} of ${proteinTarget} grams protein logged`} /></div>}
      ...
      {results && <section className="results" aria-live="polite"><div className="results-heading"><div><p className="eyebrow">TODAY&apos;S ESTIMATE</p><h2>{profile.name ? `${profile.name}'s plan` : "Your daily plan"}</h2></div><strong>{results.calories} kcal</strong></div><div className="macro-grid"><article><span>Protein</span><strong>{results.protein} g</strong></article><article><span>Carbohydrates</span><strong>{results.carbs} g</strong></article><article><span>Fat</span><strong>{results.fat} g</strong></article></div>
        <div className="estimate-protein"><div className="estimate-protein-heading"><div><p className="eyebrow">PROTEIN RECOMMENDATION</p><h3>{remainingProtein === 0 ? "Target covered" : `${remainingProtein} g remaining`}</h3></div><span>{totalProteinEaten} / {results.protein} g</span></div>{remainingProtein > 0 ? <><p className="estimate-help">Choose one option or mix smaller portions.</p><div className="visual-portions"><article><span>🥤</span><small>Whey</small><strong>{remainingProtein <= 12 ? "½" : "1"} serving</strong><em>~{Math.min(25, remainingProtein)} g protein</em></article><article><span>🍗</span><small>Chicken</small><strong>~{Math.ceil(remainingProtein / proteinPer100.chicken * 10) * 10} g</strong><em>cooked</em></article><article><span>🥩</span><small>Pork</small><strong>~{Math.ceil(remainingProtein / proteinPer100.pork * 10) * 10} g</strong><em>cooked</em></article><article><span>🥩</span><small>Beef</small><strong>~{Math.ceil(remainingProtein / proteinPer100.beef * 10) * 10} g</strong><em>cooked</em></article><article><span>🐟</span><small>Fish</small><strong>~{Math.ceil(remainingProtein / proteinPer100.fish * 10) * 10} g</strong><em>cooked</em></article><article><span>◻️</span><small>Tofu</small><strong>~{Math.ceil(remainingProtein / proteinPer100.tofu * 10) * 10} g</strong><em>firm tofu</em></article></div><p className="food-source">Check whey protein per serving on its label. Food portions are approximate.</p></> : <p className="target-covered">Your logged protein meets today&apos;s estimated target.</p>}</div>
        <div className="estimate-carbs"><div className="estimate-protein-heading"><div><p className="eyebrow">CARBOHYDRATE PORTIONS</p><h3>About {Math.round(results.carbs / 3)} g per main meal</h3></div><span>{results.carbs} g/day</span></div><p className="estimate-help">Choose one guide per meal, then adjust across your day.</p><div className="visual-portions carb-portions"><article><span>🍚</span><small>Cooked rice</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.rice * 10) * 10} g</strong></article><article><span>🍜</span><small>Cooked noodles</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.noodles * 10) * 10} g</strong></article><article><span>🥔</span><small>Potato</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.potato * 10) * 10} g</strong></article><article><span>🥣</span><small>Dry oats</small><strong>~{Math.ceil((results.carbs / 3) / carbsPer100.oats * 5) * 5} g</strong></article><article><span>🍌</span><small>Banana</small><strong>~{Math.max(1, Math.round((results.carbs / 3) / 23 * 2) / 2)}</strong><em>medium</em></article></div><p className="food-source">Approximate carbohydrate equivalents; brands, varieties and cooking methods differ.</p></div>
        <div className="supplement-plan concise-plan"><div className="section-heading compact"><span>✓</span><div><h2>Today&apos;s supplements</h2><p>Only selected inventory items.</p></div></div>{advice.length === 0 ? <div className="empty-state">No inventory items selected. Add or select products under Supplements.</div> : <div className="advice-list">{advice.map((item) => <article className={`advice-card ${item.status}`} key={item.id}><div className="advice-title"><h3>{item.name}</h3><span className="status">{statusLabels[item.status]}</span></div><b className="advice-amount">{item.amount}</b><details><summary>Why?</summary><p>{item.reason}</p></details></article>)}</div>}<details className="safety-note"><summary>Supplement safety</summary><p>Supplements can interact with medicines and health conditions. Check with a clinician or pharmacist if pregnant, managing a condition, taking medication, or unsure about combined ingredients. Vitamin K can interact with warfarin.</p></details></div>
      </section>}
    </> : <>
