...
function App() {
  ...
  return <main className="page"><section className="calculator">
    ...
    {tab === "workout" ? <>
      ...
      {proteinTarget === null ? <button className="protein-setup" type="button" onClick={() => setTab("profile")}>Add your weight and goal to see a recommended protein target →</button> : <div className="live-protein-plan"><div className="protein-summary"><article><span>Recommended</span><strong>{proteinTarget} g</strong></article><article><span>Logged</span><strong>{totalProteinEaten} g</strong></article><article><span>Remaining</span><strong>{remainingProtein} g</strong></article></div><progress className="protein-progress" max={proteinTarget} value={Math.min(proteinTarget, totalProteinEaten)} aria-label={`${totalProteinEaten} of ${proteinTarget} grams protein logged`} /><div className="suggestion-heading"><h4>{remainingProtein === 0 ? "Protein target covered" : `Suggested options for ${remainingProtein} g remaining`}</h4><p>{remainingProtein === 0 ? "No additional protein is suggested." : "Choose one, or combine smaller portions across meals."}</p></div>{remainingProtein > 0 && <div className="portion-list"><div><span>🥤 Whey</span><strong>{remainingProtein <= 12 ? "½" : "1"} serving · ~{Math.min(25, remainingProtein)} g protein</strong></div><div><span>🍗 Chicken breast</span><strong>~{Math.ceil(remainingProtein / proteinPer100.chicken * 10) * 10} g</strong></div><div><span>🥩 Lean pork</span><strong>~{Math.ceil(remainingProtein / proteinPer100.pork * 10) * 10} g</strong></div><div><span>🥩 Lean beef</span><strong>~{Math.ceil(remainingProtein / proteinPer100.beef * 10) * 10} g</strong></div><div><span>🐟 Fish</span><strong>~{Math.ceil(remainingProtein / proteinPer100.fish * 10) * 10} g</strong></div><div><span>◻️ Firm tofu</span><strong>~{Math.ceil(remainingProtein / proteinPer100.tofu * 10) * 10} g</strong></div></div>}<p className="food-source">Check the whey label for protein per serving; food amounts are approximate cooked weights.</p></div>}
